Feature: Exchange two other players' card
    ...
    Background:
        ...
        Given that Announcer has announced that it is the turn of the Troublemaker
    ...
    Scenario: (Error Flow) I choose myself as one target to swap
		...
		Then the system should not perform that request and display the warning message to me since the request is not allowed